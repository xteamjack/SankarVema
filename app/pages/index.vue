...
if (!doc.value) {
  throw createError({ statusCode: 404, statusMessage: 'Home content not found', fatal: true })
}
...
const topAchievements = computed<any[]>(() => ((achievements.value as any)?.items ?? []).slice(0, 4))
const topContributions = computed<any[]>(() => ((contributions.value as any)?.items ?? []).slice(0, 3))
...
<template>
  <div>
    <!-- Profile backdrop: your photo, clipped to the geo-shape, faint, lower-left -->
    <div
      class="pointer-events-none fixed bottom-0 left-0 -z-10 h-[88vh] w-[42vw] max-w-[560px] opacity-30"
      aria-hidden="true"
      style="
        background-image: url('/profile-photo.svg');
        background-repeat: no-repeat;
        background-position: bottom left;
        background-size: contain;
        -webkit-mask-image: url('/profile-mask.svg');
        -webkit-mask-repeat: no-repeat;
        -webkit-mask-position: bottom left;
        -webkit-mask-size: contain;
        mask-image: url('/profile-mask.svg');
    ...
    <SectionPanel id="work" index="01" eyebrow="My Work">
      <div class="max-w-2xl">
        <h2 class="font-[family-name:var(--font-display)] text-3xl tracking-tight text-[var(--color-fg)] sm:text-5xl">
          Systems I designed and shipped.
        </h2>
        <p class="mt-4 text-[var(--color-fg-muted)]">
          Production AI — not demos. Each case study walks the architecture, the decisions, and what it traded.
        </p>
      </div>
    ...
    <SectionPanel id="journey" index="02" eyebrow="My Journey">
      <div class="max-w-2xl">
        ...
        <p class="mt-4 text-[var(--color-fg-muted)]">
          From a single contributor in 1998 to AI architect today — leadership years used as ballast, not identity.
        </p>
      </div>
      <div class="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <div v-for="e in journey" :key="`${e.year}-${e.company}`" class="glass flex flex-col p-5">
          <div class="font-[family-name:var(--font-mono)] text-sm text-[var(--color-accent)]">{{ e.year }}</div>
          <div class="mt-2 font-[family-name:var(--font-display)] text-base leading-snug text-[var(--color-fg)]">{{ e.role }}</div>
    ...
    <SectionPanel id="achievements" index="03" eyebrow="Achievements">
      <div class="max-w-2xl">
        ...
        <p class="mt-4 text-[var(--color-fg-muted)]">
          National honours, Distinguished Engineer status, and trust from Fortune 500 leadership.
        </p>
      </div>
      <div class="mt-8 grid gap-4 sm:grid-cols-2">
        <div v-for="a in topAchievements" :key="a.title" class="glass flex items-start gap-4 p-5">
          <span class="mt-0.5 font-[family-name:var(--font-mono)] text-xs text-[var(--color-accent)]">{{ a.year }}</span>
          <div>
            <div class="font-[family-name:var(--font-display)] text-base leading-snug text-[var(--color-fg)]">{{ a.title }}</div>
            <div v-if="a.org" class="text-sm text-[var(--color-fg-muted)]">{{ a.org }}</div>
          </div>
        </div>
      </div>
    ...
    <SectionPanel id="contributions" index="04" eyebrow="Contributions">
      <div class="max-w-2xl">
        ...
        <p class="mt-4 text-[var(--color-fg-muted)]">
          The compounding wins are upstream, in the open. A few things I've helped build and give away.
        </p>
      </div>
      <div class="mt-8 grid gap-4 md:grid-cols-3">
        <div v-for="c in topContributions" :key="c.title" class="glass flex flex-col p-5">
          <div class="font-[family-name:var(--font-display)] text-lg text-[var(--color-fg)]">{{ c.title }}</div>
          <div v-if="c.role" class="mt-0.5 text-xs font-[family-name:var(--font-mono)] uppercase tracking-wider text-[var(--color-accent)]">{{ c.role }}</div>
          <p class="mt-3 text-sm leading-relaxed text-[var(--color-fg-muted)]">{{ c.note }}</p>
        </div>
      </div>
    ...
    <SectionPanel id="advisory" index="05" eyebrow="Work With Me">
      <div class="glass relative overflow-hidden p-8 sm:p-12">
        <div
          ...
          style="background: radial-gradient(circle, color-mix(in oklab, var(--color-accent) 30%, transparent), transparent 70%);"
          aria-hidden="true"
        />
        ...
        <p class="mt-5 max-w-2xl text-lg text-[var(--color-fg-muted)]">
          I'm the architect-on-call for leadership teams building AI that has to ship — not just demo.
          The first conversation is a free 30-minute diagnosis. I won't sell; I'll diagnose.
        </p>

        <div class="mt-8 grid gap-4 sm:grid-cols-3">
          <div v-for="x in [
            { t: 'Architecture review', d: 'What\'s sound, what breaks in year two, what to stop building now.' },
            { t: 'Fractional Chief AI Architect', d: 'Embedded with your exec team for a 6–12 month transformation.' },
            { t: 'Founder mentorship', d: 'AI product architecture, technical hiring, scaling without the standard mistakes.' },
          ]" :key="x.t" class="rounded-xl border border-[var(--color-hair)] p-4">
            <div class="text-sm font-semibold text-[var(--color-fg)]">{{ x.t }}</div>
            <div class="mt-1 text-sm text-[var(--color-fg-muted)]">{{ x.d }}</div>
          </div>
        </div>
