...
hero: A voice-and-video AI that conducts technical, HR, and managerial screenings end-to-end &mdash; with sub-second response and structured evaluation output.
draft: true
---
...
the rubric is consistent, the conversation is bounded &mdash; in other words,
...
2. **Structure.** Hiring teams don't need transcripts &mdash; they need a
...
- **Reasoning core:** LLM constrained to a custom evaluation **JSON schema**
  per candidate, per round &mdash; so the output is queryable, comparable, and
...
- **Round routing:** technical / HR / managerial flows are configuration,
  not code &mdash; recruiters compose new flows without engineering.
...
| Structured JSON output over free text | Comparability across candidates | Some loss of qualitative texture &mdash; mitigated with a `notes` field |
| Sub-second target | Conversational naturalness | Forced eager inference and parallelization &mdash; meaningful compute cost |
