...
excerpt: An opinionated tour of the patterns that have survived contact with production across my last five agent builds &mdash; and the ones I've stopped reaching for.
draft: true
status: outline
---
...
twin builds &mdash; the patterns I keep reaching for, why, and the ones that
...
## Pattern 2: Tools are the API surface &mdash; treat them like one
...
- **Pure ReAct loops with unlimited horizon** &mdash; impressive in benchmarks,
  unwieldy in production. Cap the steps; orchestrate the rest.
- **One mega-prompt with everything in it** &mdash; readable for a week, then a
  liability forever.
- **"Let the agent decide" as a UX choice** &mdash; users want predictability.
...
- Budget for the eval harness like you'd budget for tests &mdash; because that's
  what it is.
